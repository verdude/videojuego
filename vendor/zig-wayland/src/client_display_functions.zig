...
pub inline fn dispatch(display: *Display) posix.E {
    return errnoFromWlResult(ffi.client.wl_display_dispatch(display));
}

pub inline fn dispatchQueue(display: *Display, queue: *client.wl.EventQueue) posix.E {
    return errnoFromWlResult(ffi.client.wl_display_dispatch_queue(display, queue));
}

pub inline fn dispatchPending(display: *Display) posix.E {
    return errnoFromWlResult(ffi.client.wl_display_dispatch_pending(display));
}

pub inline fn dispatchQueuePending(display: *Display, queue: *client.wl.EventQueue) posix.E {
    return errnoFromWlResult(ffi.client.wl_display_dispatch_queue_pending(display, queue));
}

pub inline fn roundtrip(display: *Display) posix.E {
    return errnoFromWlResult(ffi.client.wl_display_roundtrip(display));
}

pub inline fn roundtripQueue(display: *Display, queue: *client.wl.EventQueue) posix.E {
    return errnoFromWlResult(ffi.client.wl_display_roundtrip_queue(display, queue));
}

pub inline fn flush(display: *Display) posix.E {
    return errnoFromWlResult(ffi.client.wl_display_flush(display));
}
...
pub inline fn readEvents(display: *Display) posix.E {
    return errnoFromWlResult(ffi.client.wl_display_read_events(display));
}

/// Wayland client functions use the libc convention: -1 indicates failure and
/// the actual error is stored in the thread-local errno value. Zig's Linux
/// posix.errno now expects a raw syscall result, so passing the c_int directly
/// no longer has the right type or semantics.
inline fn errnoFromWlResult(result: c_int) posix.E {
